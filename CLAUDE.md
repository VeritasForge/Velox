...
**Treesitter v0.9.3 directive patch** - `nvim-treesitter` is pinned to tag `v0.9.3` (see `treesitter.lua`), whose bundled query directives (`set-lang-from-info-string!`, `downcase!`) assume `match[capture_id]` is a single `TSNode`. Neovim 0.11+ always wraps query captures in a list, so those directives crash with "attempt to call method 'range' (a nil value)" on markdown fenced-code-block info strings and bash heredocs. `treesitter.lua`'s `config` function re-registers both directives with `force = true`, unwrapping the list before calling `get_node_text`. If the pinned tag is ever upgraded past the point upstream fixes this, these overrides can be removed.
